...
REDIPS.drag = (function () {
	...
	handler_onmouseup = function (e) {
		...
		// if ordinary element was clicked and left button was released, but element is placed inside unmovable table cell
			...
			// remove object if destination cell has "trash" in class name
				...
				target_elements = target_cell.getElementsByTagName('td');
			...
			// else append object to the cell and call myhandler_dropped 
				...
				//刘勇修改于2013-12-13
				if(cell !== cell_source) {
					var tds = getTdsByindex(cell);
					if(tds.length == 1) {
						var key = $(tds[0]).attr('key');
						if(key) {
							AJAX.wemenu.referenced({'key':key},function(data){
								//将两列对调
	...
	//刘勇修改于2013-12-16日
	handler_aaa = function() {
		var tds = getTdsByindex(cell_source);
		if(tds.length == 1) {
			var key = $(tds[0]).attr('key');
			if(key) {
				AJAX.wemenu.referenced({'key':key},function(data){
