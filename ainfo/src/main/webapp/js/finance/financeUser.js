	...
	AJAX.account.query(pageinfo, function(data){
		...
		if(lines.length > 0){
			...
			for(var i=0; i<lines.length; i++){
				...
				lineHtml += '<div class="line" onmouseover="lineOnMouseOver(this, ' + i + ', ' + line.uid + ')">';
				...
				lineHtml += '			<div class="line_up" style="top: 20px;">';
				...
				lineHtml += '				<span>累计充值:' + line.rechargeTotal + '</span>';
				lineHtml += '			</div>';
				lineHtml += '			<div class="line_down">';
				...
				lineHtml += '		<div class="line_contacts_platform">';
				lineHtml += '			<div class="line_up" style="top: 20px;">';
				lineHtml += '				<span style="font-size:20px;">存鱼:' + line.balance + '</span>';
				lineHtml += '			</div>';
				lineHtml += '			<div class="line_down">';
				lineHtml += '				<span></span>';
				lineHtml += '			</div>';
				...
				lineHtml += '			<div class="line_operate_button" onclick="queryTradeList(' + i + ', ' + line.uid + ')" style="width: 70px;">漂币记录</div>';
				lineHtml += '			<div class="line_operate_button" onclick="storeDetail(' + i + ', ' + line.balance + ')" style="width: 70px;">存鱼情况</div>';
				if(line.flag == 1){
...
//	if(lineData.portriat.substr(0, 4) == "WXIN"){
//		lineData.portriat = "http://wx.qlogo.cn/mmopen/" + lineData.portriat.substr(4);
//	}else{
//		lineData.portriat = RES_PROC_URL + lineData.portriat;
//	}
...
function queryTradeList(index, uid){
	var currSite = currentPageData[index];
	//分页查询会员信息列表
	currQueryUserParam.type = 1;
	currQueryUserParam.qtype = 15;
	currQueryUserParam.uid = uid;
	...
	createDialogPopupOnlyCancel('queryTradeList_dialog', bodyWidth-50, bodyHeight-120, currSite.nickname + "漂币记录");
	queryList();
	...
	createcombobox('styleSelect', recordStyleArray, function(sData){
		$("#check_main").html("");
		currQueryUserParam.qtype = sData.value;
		currQueryUserParam.pagenum = 1;
		queryList();
	});
...
function queryList(){
	AJAX.account.list(currQueryUserParam, function(ret){
		...
		var rowNumberBase = (currQueryUserParam.pagenum - 1) * currQueryUserParam.pagesize;
		if(rows.length > 0){
			for(var i=0; i<rows.length; i++){
				var row = parseListData(rows[i]);
				html = "";
				html += '		<div class="cselecter_table_body">';
				html += '			<div style="width:  5%; text-align: right;">' + (i + 1 + rowNumberBase) + '</div>';
 ...
 * 存鱼情况
 */
function storeDetail(index, balance){
	var currSite = currentPageData[index];
	currQueryUserParam.fishSiteId = currSite.fishSiteId;  //设置钓场id
	currQueryUserParam.uid = currSite.uid;  //设置用户id
	//分页查询会员信息列表
	currentFeeType = [];
	var html = "";
	...
	html += '		<p style="top: 0px;" id="storeTotal">' + balance + '</p>';
	html += '	</div>';
	html += '	<div class="selecter_lables selecter">';
	html += '		<div class="selecter_lable" onclick="switchSelecter(this, 3, switchCallBack)" style="width:187px">存鱼情况</div>';
	html += '		<div class="selecter_lable" onclick="switchSelecter(this, 4, switchCallBack)" style="width:187px">存鱼流水</div>';
	html += '	</div>';
	html += '	<div class="selecter_mains selecter" style="height:172px">';
	html += '		<div class="selecter_main" id="selecter-3" style="height:490px"> <!--动态增加用户数据--> </div>';
	html += '		<div class="selecter_main" id="selecter-4" style="height:490px"> <!--动态增加用户数据--> </div>';
	html += '	</div>';
	html += '</div>';
	$(html).appendTo($(document.body));
	
	createDialogPopupOnlyCancel('editfeeType_dialog', 400, 700, currSite.nickname + "存鱼信息",function(){
		//
	});
	switchSelecter(null, 3, switchCallBack);
...
function switchSelecter(obj, sp_id, __callback){
	...
	if(obj == null){
		obj = $(".selecter_lables").children().eq(sp_id - 3);
	}
...
function switchCallBack(sp_id){
	...
	currQueryUserParam.pagesize   = 18;
...
function queryUserFinance(sp_id){
	if(sp_id == 4){
		html = '<center style="color: rgba(255, 0, 0, 0.21);">需求不明确，明确后加对接口！</center>';
			...
			if(sp_id == 3){
				for(var i=0; i<rows.length; i++){
					...
					if(typeof row.portriat != 'undefined' && row.portriat.substr(0, 4) == "WXIN"){
						...
						row.portriat = "../../images/frame_images/p65.png";
					}
					html = "";
					...
					html += '			</div>';
					html += '			<div class="check_main_field_text" title="' + row.userId + '">' + row.userId + '</div>';
				...
				for(var i=0; i<rows.length; i++){
					...
					html += '						<img src="' + row.portriat + '" onload="imgLoad(this)"/>';
					html += '					</div>';
					html += '					<div style="float: left; margin-top: 5px;" title="' + row.userId + '">' + row.userId + '</div>';
					html += '				</div>';
					...
					html += '				<div class="line_up"   style="position: relative; top:  8px; float: right; margin-right: 15px; color: red;">';
					html += '					<span>' + row.tradeMoney + '</span>';
					html += '				</div>';
					html += '				<div class="line_down" style="position: relative; top: 35px; float: right; right: -31px;">';
