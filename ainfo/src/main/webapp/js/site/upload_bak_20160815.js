/**
 * 可用的脚本文件，命名规则是 id/0-8.媒体后缀
 */

var currentUpdateSiteId = -1;
...
function send_request(){
    ...
    if (xmlhttp!=null){
        ...
        xmlhttp.send('{"bucket":"fs-profile","dir":"' + currentUpdateSiteId + '/"}');
...
var uploader = new plupload.Uploader({
	...
	init                :{
		...
		FilesAdded: function(up, files) {
			...
			if(currentImgCount == 9 && currendChooseFilesCount > 1){
				//$(".img_element").remove();
				_E.window.show("目前多媒体文件位置已满，不支持多选，请选择一个位置，再修改！");
				return;
			}
		...
		BeforeUpload: function(up, file) {
			//console.info(Math.round(new Date()) + " Judge fiel.currentImgSet");
		...
		FileUploaded: function(up, file, info) {
            if (info.status == 200){
            	...
            	eval("currentImgArray.a" + file.currentImgSet + ".src='" + file.ossPathFile + "'");
                console.info('upload to oss success, get url is:' + uParams.url + "/" + file.ossPathFile);
            	...
            	$("#u1443").attr("resources", resources);
            	$("#u1443").attr("mainimg", currentImgArray.mainImg);
            }
            else{
 ...
 * 新建钓场得到钓场ID后，上传文件
 */
function newSiteUploadFiels(fishSiteId, _callback){
	//设置当前钓场ID
...
function addImg(imgSrc, img_index){
	...
	if(imgSrc != null && imgSrc != undefined){
		...
		setSrc2Img(img_index, RES_PROC_URL + imgSrc, null, null, false, null);
	}else{
		var html = "";
...
function addImgPreview(file ,imgSrc, imgIndex){
	...
	if(currendChooseFilesCount > 1){
        //console.info(Math.round(new Date()) + " " + file.id + " addImgPreview ()");
		//console.info("addImgPreview->uploader.files.length", uploader.files.length);
		//var imgIndex = getImgRightSet(null, file);
...
function setSrc2Img(imgIndex, imgSrc, imgId, imgName, isPreview){
	...
	if(imgSrc != undefined && imgSrc.indexOf(RES_PROC_URL) != -1){
		imgSrc = imgSrc.substr(imgSrc.indexOf(RES_PROC_URL) + RES_PROC_URL.length);
	}
